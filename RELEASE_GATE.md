...
3. **Reproducibility gate:** set concrete `OLLAMA_IMAGE_TAG` and `N8N_IMAGE_TAG` values. `latest`, `stable`, `nightly`, branch names, and empty values are rejected.
4. **Linux Docker/Compose gate:** on GitHub's Linux runner, render the Compose file, start both pinned images, and prove the Ollama and n8n health endpoints respond.
5. **Security review gate:** create the n8n owner account before exposure; keep n8n and Ollama on localhost unless a reviewed TLS reverse proxy is present; protect KnowledgeDigest with TLS/authentication and a firewall; review `.env` without committing it.
...
The uploaded evidence belongs to that workflow run and commit. Re-run the gate whenever the commit or either image tag changes.
...
- [ ] Backup and rollback instructions are reviewed for the exact component versions.
